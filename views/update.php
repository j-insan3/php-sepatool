...
//Vul variabelen vanuit POST
 ...
 //Voer query uit
 if ($stmt = $link->prepare("UPDATE debtor SET member=?, member_lastname=?, Adres=?, PC=?, City=?, Phone=?, Email=?, BirthDate=?, NBB=?, Comment=?, debtor_name=?, debtor_account_IBAN=?, debtor_mandate=?, debtor_mandate_date=?, member_type_id=?, creditor_id=? WHERE id='$id'")) {
